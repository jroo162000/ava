...
function _plannedStage(stage, index, startAt = 1) {
  const n = startAt + index;
  return {
    n,
    title: String(stage?.title || `Stage ${n}`).slice(0, 120),
    goal: String(stage?.goal || '').slice(0, 1600),
    doneWhen: String(stage?.done_when || stage?.doneWhen || 'The stated result is evidenced and complete.').slice(0, 600),
    needsTools: stage?.needs_tools !== false,
    status: 'pending',
    attempts: 0,
    result: '',
    error: '',
...
function _fallbackPlanStages(goal, options = {}, startAt = 1, failedApproach = '') {
  const criteria = Array.isArray(options.acceptanceCriteria)
    ? options.acceptanceCriteria.map(value => String(value || '').trim()).filter(Boolean)
    : [];
  const alternateRoute = failedApproach
    ? ` Use a different route from this failed approach and do not merely repeat it: ${String(failedApproach).slice(0, 500)}`
    : '';
  return [_plannedStage({
    title: failedApproach ? 'Complete remaining goal' : 'Complete and verify goal',
    goal: `${String(goal || '').trim()}${alternateRoute}`,
    done_when: criteria.join('; ') || 'The requested result is complete and supported by successful tool receipts or other direct evidence.',
    needs_tools: options.needsTools !== false,
  }, 0, startAt)];
}
...
async function planGoal(goal, options = {}) {
  ...
  try {
    ...
    const stages = (j && Array.isArray(j.stages) ? j.stages : [])
      ...
      .map((s, i) => _plannedStage(s, i));
    if (stages.length) return stages;
    logger.warn('[workflow] planner returned no usable stages; using deterministic fallback');
    return _fallbackPlanStages(goal, options);
  } catch (e) {
    logger.warn('[workflow] planGoal failed; using deterministic fallback', { error: e.message });
    return _fallbackPlanStages(goal, options);
  }
}
...
async function replan(wf, failedIdx, reason) {
  ...
  try {
    ...
    const stages = (j && Array.isArray(j.stages) ? j.stages : []).filter(s => s && s.goal).slice(0, 10)
      .map((s, i) => _plannedStage(s, i, failedIdx + 1));
    if (stages.length) return { stages };
    logger.warn('[workflow] replanner returned no usable stages; using deterministic fallback', { failedStage: failed.title });
    return {
      stages: _fallbackPlanStages(wf.goal, { acceptanceCriteria: wf.acceptanceCriteria }, failedIdx + 1,
        `${failed.goal}. Failure: ${String(reason || '').slice(0, 400)}`),
    };
  } catch (e) {
    logger.warn('[workflow] replan failed; using deterministic fallback', { error: e.message });
    return {
      stages: _fallbackPlanStages(wf.goal, { acceptanceCriteria: wf.acceptanceCriteria }, failedIdx + 1,
        `${failed.goal}. Failure: ${String(reason || '').slice(0, 400)}`),
    };
...
export default {
  ...
  _internals: {
    ...
    _isSubstantiveStageResult, _isReceiptOnlyRejection, _plannedStage, _fallbackPlanStages,
    _saveAll, _loadAll,
  },
};
