...
const PROPOSAL_REVIEWER_NAMES = Object.freeze({
  claude: 'anthropic-api-reviewer',
  openai: 'openai-api-reviewer',
  gemini: 'google-api-reviewer',
  deepseek: 'deepseek-api-reviewer',
  grok: 'xai-api-reviewer',
  groq: 'groq-api-reviewer',
});

export function proposalReviewerPanel() {
  const models = modelConfig.proposalReviewerModels();
  return modelConfig.providerOrder()
    .filter(provider => provider !== 'local' && Array.isArray(models[provider]) && models[provider].length)
    .map(provider => ({
      provider,
      reviewer: PROPOSAL_REVIEWER_NAMES[provider] || `${provider}-api-reviewer`,
...
async function runProposalReviewer({ reviewer, model, call, file, reason, find, replace, diff, ctx }) {
  ...
  const candidates = (Array.isArray(model) ? model : [model]).filter(Boolean);
  ...
  for (const candidate of candidates) {
    try {
      ...
      return normalizeReview(r.content || r.text || '', reviewer, r.model || candidate || reviewer);
    } catch (e) {
      failures.push(`${candidate || 'default'}: ${String(e.message || e).slice(0, 240)}`);
    }
  }
  return {
    reviewer,
    model: candidates.filter(Boolean).join(' -> ') || reviewer,
    recommendation: 'unavailable',
    reason: failures.join(' | ').slice(0, 600),
    risks: [],
  };
}
...
async function reviewProposal({ file, reason, find, replace, diff }) {
  ...
  // Ask every cloud provider in AVa's configured chain independently. Each provider
  // tries its strongest current model first and records the exact model that answered.
  const panelReviews = await Promise.all(proposalReviewerPanel().map(spec =>
    runProposalReviewer({
      reviewer: spec.reviewer,
      model: spec.models,
      call: (opts) => llmService.createCompletionForProvider(spec.provider, opts),
      file,
      reason,
      find,
      replace,
  ...
  reviews.push(...panelReviews);

  let available = reviews.filter(r => r.recommendation !== 'unavailable');
  // If every independent cloud reviewer is unavailable, retain the resilient chain as
  // a final continuity fallback so a proposal is never silently exposed as unreviewed.
  if (!available.length) {
