...
describe('proposal discussion versus mutation intent', () => {
  ...
  test('leaves explicit workflow approval to the workflow router', async () => {
    const selfMod = jest.spyOn(pythonWorker, 'selfMod');

    const result = await handleSelfModVoice('both workflows say waiting for user you are approved to move forward with both workflows');

    expect(result).toBeNull();
    expect(selfMod).not.toHaveBeenCalled();
  });

  test('does not treat yes after a workflow permission prompt as proposal approval', async () => {
    jest.spyOn(conversationLogger, 'getRecentHistoryAcrossDays').mockReturnValue([
      {
        direction: 'assistant',
        content: 'I need your input to continue the sys_ops workflow. This stage needs permission to use fs_read. Do you approve this exact action?',
      },
    ]);
    const selfMod = jest.spyOn(pythonWorker, 'selfMod').mockImplementation(async ({ action }) => {
      if (action === 'list_pending') {
    ...
    expect(await handleSelfModVoice('yes')).toBeNull();
    expect(selfMod.mock.calls.map(([request]) => request.action)).toEqual(['list_pending']);
  });

  test('routes a generic action approval to a linked waiting workflow despite background proposal messages', async () => {
    const selfMod = jest.spyOn(pythonWorker, 'selfMod');

    const result = await handleSelfModVoice('yes i approve the action', { workflowWaiting: true });

    expect(result).toBeNull();
    expect(selfMod).not.toHaveBeenCalled();
  });

  test('keeps an explicit proposal ID on the proposal path while a workflow waits', async () => {
    const selfMod = jest.spyOn(pythonWorker, 'selfMod').mockImplementation(async ({ action }) => {
      if (action === 'list_pending') {
        return { status: 'ok', pending: [{ id: '75cc35e1', status: 'pending', file: 'eventLedger.js' }] };
      }
    ...
    const result = await handleSelfModVoice('approve proposal deadbeef', { workflowWaiting: true });

    expect(result).toMatch(/proposal id deadbeef/i);
    expect(result).toMatch(/did not approve or reject anything/i);
    expect(selfMod.mock.calls.map(([request]) => request.action)).toEqual(['list_pending']);
  });
});
