...
async function _verifyStage(stage, state, result) {
  ...
  const system = [
    ...
    'Decide only from the supplied tool receipts and result. A claim without evidence is not complete.',
  ...
  try {
    ...
    if (verdict && typeof verdict.accepted === 'boolean') return { accepted: verdict.accepted, reason: String(verdict.reason || '').slice(0, 800), receipts };
  } catch (error) {
...
async function _runStage(wf, idx) {
  ...
  if (outcome.kind === 'waiting_user') {
    return { ok: false, waitingUser: true, question: outcome.question, result: outcome.question, steps: state ? state.step_count : 0, verification: null };
  }
  ...
  try {
    ...
    while (true) {
      ...
      if (res.waitingUser) {
        ...
        _checkpoint(wf);
        _announce(`I need your input to continue "${String(wf.goal).slice(0, 60)}": ${question}`);
        return;
      }
...
export default {
  ...
  _internals: {
    ...
    _classifyAgentOutcome, _applyWorkflowInput, _saveAll, _loadAll,
  },
};
