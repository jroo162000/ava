...
function successfulHistoryResult(state) {
  const results = (state?.history || []).flatMap(entry => {
    ...
    const evidence = toolResultEvidence(result, 2200) || String(result?.message || '').trim();
    return [{
      title: `${tool} receipt`,
...
async function successfulRunAnswer(state, fallbackTool, fallbackResult) {
  ...
  return answerFromSuccessfulTool({
    tool: 'completed tool receipts',
    result: combined,
    synthesize: () => synthesizeAnswer(state.goal, 'completed tool receipts', combined, state.localPriority),
  });
}
...
export default {
  ...
  _internals: {
    ...
    autonomyCategoryForTool, successfulHistoryResult, decide,
  }
};
