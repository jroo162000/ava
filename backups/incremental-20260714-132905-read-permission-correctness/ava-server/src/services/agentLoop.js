...
function isReadOnlyObservationTool(name = '') {
  return /^(?:fs_find|fs_read)$/.test(String(name || ''));
}
    ...
    switch (decision.decision) {
      case DecisionType.TOOL_CALL:
        ...
        if (state.readOnly) {
          ...
          const safeObservation = isReadOnlyObservationTool(decision.tool);
        ...
        if (_priorRun) {
          ...
          state.final_result = state.final_result || (_spinOk
            ? await successfulToolAnswer(state, decision.tool, result)
        ...
        // browser tab. General: any tool result carrying an http(s) preview_url becomes a web card.
        try {
      ...
      if (decision && decision.decision === 'tool_call' && decision.tool) {
        ...
        if (_prior && _prior.result) {
          ...
          state.final_result = state.final_result || (_ok
            ? await successfulToolAnswer(state, decision.tool, _prior.result)
...
export default {
  ...
  _internals: {
    ...
    errorMessageForResult, classifyConfirmationResponse, isReadOnlyObservationTool, decide,
  }
};
