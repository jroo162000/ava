...
describe('agentLoop long-horizon hooks (Tier 2 #14)', () => {
  ...
  test('read-only workflows recognize filesystem reads as observations, not writes', () => {
    const isObservation = agentLoop._internals.isReadOnlyObservationTool;
    expect(isObservation('fs_read')).toBe(true);
    expect(isObservation('fs_find')).toBe(true);
